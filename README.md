...
A：再上一档——**全代理版订阅**：后台同一处点「🚚 全代理版」复制地址（即原地址加 `?relay=2`），在播放器里重新添加订阅。兼容版只替播放器走完跳转，下发的清单里仍是 CDN 的**绝对地址**；极影视这类播放器连清单里的绝对地址都取不动（表现同样是转圈失败，但把那条 CDN 地址单独填进播放器却能播）。全代理版把清单里的地址全部改成**本机同源相对地址**，视频分片也经服务器转发，播放器只跟本机打交道。代价是视频流经服务器（NAS 与电视同网时多走一段内网带宽，外网下行量不变），**能用兼容版就别用它**（issue #98）。
...
| 上面这些播放器**用了兼容版仍播不了** | **全代理版订阅**（`/interface.m3u?relay=2`，后台可一键复制；分片经服务器转发） |
