...
> 一个基于 Docker 部署的**模块化 IPTV 直播源管理与分发系统**：把各路直播内容——平台**抓取模块**（内置咪咕视频、哔哩哔哩直播两个模块）、内置精选频道、你自己的直连源与 m3u/txt 订阅——聚合成一份干净的播放列表（m3u / txt），配上聚合 EPG 节目单与可视化管理后台。
>
> - 🧩 **模块化源管理** — 每个内容平台是一个独立「抓取模块」，卡片式开关 + 配置 + 健康状态，按需启用，互不牵连
> - 📺 **开箱即用** — 部署完即有 300+ 频道（央视 / 卫视 / 地方台）与内置**精选频道**订阅（央视 / 港澳台 / 地方 / 国际），另有**纬来体育、Red Bull、4K 卫视及 HBO / CNN / BBC 等海外频道**，零配置可看
> - 🏆 **体育赛事** — 赛事直播（昨天 / 今天 / 明天）与 B 站分区热门直播间（模块一键开启即用，默认「赛事」分区），打开就是正在打的比赛
> - 🎫 **高画质 / 会员内容** — 在对应模块卡片里配置平台账号即可解锁（支持扫码登录 / 书签工具一键获取，免抓包）
> - 🔗 **自定义源 / 订阅** — m3u8 直链、网页播放页自动抓取、m3u/m3u8/txt 订阅批量导入
> - 📡 **进阶玩法** — EPG 多源聚合、多配置档（一台服务给每台电视各一套频道编排）、用户令牌（一人一源、可单独吊销）
<!-- CURRENT_RELEASE_NOTES:START -->
#### 🆕 v4.0.0 更新内容

本次为「**源模块化**」大版本：咪咕从写死的内核收编为第一个「抓取模块」，新增哔哩哔哩直播模块，全部内容来源统一进顶层「🌐 源管理」。**升级完全向后兼容**——存量配置首次启动自动迁移、老环境变量继续生效，唯一可感知的变化是抓取频道的分组归一（见下）。

- 🆕 **全新「🌐 源管理」与抓取模块架构**：抓取模块、内置源、精选频道订阅、自定义源全部聚合在一页卡片网格，每张模块卡片自带**独立开关 + 配置 + 健康状态**（上次抓取时间 / 频道数 / 报错原因）。原「系统配置」里的平台账号 / 画质 / H.265 / HDR / 就近取流归位到对应模块卡片（**存量配置自动迁移，`muserId` / `mtoken` / `mrateType` / `menableH265` 等环境变量继续生效**），「获取账号」书签工具也随之搬进卡片；后台顶部有搬家指路条帮存量用户找到新位置
- 🆕 **哔哩哔哩直播模块（一键开启即得正在打的比赛）**：模块默认关闭，在「源管理」打开开关后自动抓取所选分区（默认「赛事」）人气前 8 的直播间（带人气下限，挡掉同场比赛的多机位小号）；也可手填房间号 / 直播间地址 / b23.tv 短链固定看某个主播，未开播自动跳过。SESSDATA 支持**扫码登录**一键写入（也有手动获取指引），不填可看「超清」、填了解锁「原画」；可选优先 HLS / 优先 H.264（老盒子更稳）与播放缓冲；环境变量 `mbiliSessdata` 可注入登录态
- 🆕 **频道级播放选项（#EXTVLCOPT）全链路贯通**：解析、回读、生成均支持按频道携带 Referer / User-Agent / 缓冲等播放参数——B 站这类需要请求头的直链源在 VLC 系播放器里开箱能播，订阅导入的第三方源带的 #EXTVLCOPT 也不再丢失
- 🛠️ **抓取频道分组归一**：同一频道只出现在一个分组（按固定分组优先级归一、「先出现者胜」去重），分组顺序沿用上游返回顺序，播放列表更干净。**注意**：分组分布与旧版略有不同，做过个性化分组编排的用户升级后可进「我的频道」核对一遍
- 🐛 **抓取核心稳定性大扫除**（多为长期潜伏问题）：
  - token 自动续期改用墙钟时间戳，修掉「更新间隔不整除 720 小时时永不续期」；续期也不再被容器重启 / 后台保存操作反复触发
  - 平台接口暂时不可达时改用上一轮缓存，已抓到的频道不再被静默从播放列表抹掉；节目单接口一次瞬时故障不再让整轮更新崩掉；体育赛事接口不可达时退回缓存并给出明确日志
  - 设了「启动时不更新」的用户：重启后抓取频道整批消失、体育赛事重复追加一份，两个问题都已修复
  - 播放请求遇到异常参数 / 上游错误时返回明确错误，不再让播放器挂死转圈；网页抓取的浏览器显式路径失败时能自动回退
- 🐛 **后台修复一批**：修改访问密码后不再卡在 403（跳转按新入口正确拼接）；「保存并重启」在访问入口变更时正确执行重启、并探活确认服务拉起后再跳转；后台保存不再复活已删除的内置订阅源；表单新增「有未保存的改动」标记与保存提示，即时落盘的控件不再一声不吭；模块卡片的凭据（token / SESSDATA）支持**一键清除**；模块配置表单只提交改过的字段——用环境变量部署的用户在后台点一次保存，不会再把 env 值固化成文件值（改 compose 继续生效）
- 🔒 **咪咕 token 不再被 /api/system-config 明文回显**：账号收进模块后凭据只回「有没有值」（后台显示掩码），旧接口的明文回显一并关闭——未设访问密码的部署，同网段设备不再能一个 GET 就读走等同登录态的 token（system-config.json 文件内容保持原样，供回滚）
- 🩹 **兼容版订阅再进一步**：多码率 master 清单拍平为媒体清单直出，去掉播放器侧的嵌套跳转，老电视盒兼容性更好（issue #98）
- 🗑️ **撤掉「启用抓取模块」总开关**：每个模块卡片上的开关就是唯一真相，原总开关的关闭状态升级时自动折进各模块开关，无感迁移；内置订阅源也不再混在「自定义源」列表里，只保留一个总开关
...
- **🧩 平台抓取模块** — 自动抓取内容平台的直播频道（CCTV、卫视、体育赛事、B 站直播间等），模块化设计：每个平台一张卡片，独立开关、独立配置、健康状态一目了然
- **🔗 外部源管理** — 外部 m3u8 直链、网页播放页自动抓取、m3u/m3u8/txt 订阅批量导入
...
> 🧩 **平台账号、画质等内容相关配置不用写进 compose**：部署完成后进管理后台「**🌐 源管理**」，在对应模块卡片里配置即可（支持扫码登录 / 书签工具一键获取账号），保存即时生效。习惯环境变量的进阶用户仍可用变量注入，见下方[完整配置表](#-配置说明)。

> 💾 **数据持久化（强烈建议）**：上面的 `volumes` + `mdataDir` 会把系统配置、平台账号、外部订阅源、我的频道（分组顺序/隐藏/归类/排序）等都存到宿主机的 `./data` 目录。**不挂载的话，`docker compose down`、重建容器或 `docker compose pull` 升级镜像时这些配置会全部丢失、恢复默认。**
>
> 📤 **日常备份 / 换机迁移最简单的方式**：后台「设置 → 系统配置 → 🧳 配置备份 / 迁移」一键**导出全部配置**为单个文件，新机器部署后**导入**即可恢复（含各配置档 / 用户令牌；`logos/` 自定义台标需单独拷贝）。摸不到数据卷目录的 NAS 用户（极空间等）推荐用这个。
>
> 若你之前已部署且容器里已有配置，升级前先备份出来再挂卷。**也可以直接备份整个数据目录**（包含系统配置、外部订阅源、我的频道、多配置档 `my-playlist-config.*.json`、账号、EPG 源、台标缓存等全部文件，单拷个别 json 容易漏）：
>
> ```bash
> mkdir -p ./data
...
# 或指定版本
docker pull akiralereal/iptv:4.0.0
```
...
> `-v` 把配置持久化到宿主机 `./data`，删除/重建容器、升级镜像都不丢配置；省略它则配置只存在容器的匿名卷里，换容器即丢。平台账号等内容配置部署后进后台「🌐 源管理」设置。
...
通过管理后台可以：
- ✅ 查看所有频道列表，按配置档编排分组 / 隐藏 / 排序
- ✅ 管理全部内容来源：抓取模块开关与配置、内置源、自定义源与订阅
- ✅ 配置系统参数（公网地址、访问密码、EPG 等）
- ✅ 一键更新播放列表
...
A：这是 **H.265（HEVC）编码兼容性**问题——部分平台的直播流默认是 H.265，而部分播放器（如飞牛影视内置播放器）不支持 H.265 解码，只能解出音频、没有画面。解决办法：

1. 进后台「**🌐 源管理**」，打开对应抓取模块的配置，取消勾选「**启用 H.265 原画**」（哔哩哔哩模块默认已「优先 H.264」），保存即清掉旧的流地址缓存、**即时生效**，无需重启；
...
**Q：频道列表能加载、台标都正常，但播放一直转圈，转一会就失败（极空间极影视、部分老电视盒）？**

A：这是**播放器不跟随 302 跳转**导致的——抓取模块的频道地址靠一次 302 跳转指向真实 CDN 流，绝大多数播放器会自动跟随，但极空间「极影视」等少数播放器不会，表现就是列表正常、播放转圈失败。解决办法：改用**兼容版订阅**——

1. 进后台「我的频道」顶部订阅地址栏，点「**🩹 兼容版**」复制兼容版订阅地址（也就是原地址加 `?relay=1`）；
2. 在极影视等播放器里用这个地址重新添加订阅即可。

兼容版由服务器直接送出播放清单（播放器无需跟随任何跳转），视频流仍由播放器直连 CDN、不占服务器带宽。注意兼容版的流量节点由**服务器网络**选择，适合服务器与观看设备在同一网络的场景；若你正因跨运营商卡顿开启了「客户端就近取流」，两者不能同时受益（同一订阅二选一）。其他播放器无需换用兼容版（issue #98）。
...
- ✅ **多种直播源，统一管理**
  - 平台抓取模块（300+ 频道：CCTV、卫视、地方台、体育赛事、B 站热门直播间）
  - 内置精选频道订阅源（央视 / 港澳台 / 地方 / 国际）
  - 外部自定义直播源（m3u8 直链 / 网页自动抓取）
  - 订阅模式（m3u/m3u8/txt 播放列表批量导入）
  
- ✅ **画质与登录态按模块配置**
  - 免费内容零配置可看（不登录时画质有平台侧限制，如 540p / 超清；登录免费账号可再高一档）
  - 在模块卡片里配置平台账号即可解锁高画质（蓝光 / 原画 / 4K）与会员内容
  - 支持扫码登录 / 书签工具一键获取，免手动抓包
  
- ✅ **回看功能** - 支持当天节目回看
  
- ✅ **Web 管理后台**
  - 🧩 抓取模块卡片：开关、配置、健康状态一页管完
  - 📺 自动抓取网页中的 m3u8 播放地址
  - 🔧 手动添加已知的播放地址
  - 🔄 支持独立自动刷新功能
  - ⚡ 快速模式：删除/修改源 1-2 秒内生效
  
- ✅ **自动更新节目单（EPG）** - 定时同步最新节目信息，支持多源聚合
...
1. 配置平台账号后请注意账号安全，令牌等同登录态、请勿外传
2. 部分平台的频道需要中国大陆网络环境才可正常访问

### 平台账号与画质

- **零配置可用**：不配任何账号即可观看全部免费内容（免费画质上限由各平台决定）
- **解锁高画质 / 会员内容**：进后台「**🌐 源管理**」，点开对应模块卡片的配置——
  - 支持**扫码登录**（哔哩哔哩）：手机 App 扫一下，登录态自动写入，无需抓包
  - 或用卡片里附带的**书签工具 / 手动获取指引**自行提取（视平台而定）
- 账号信息保存在你自己的部署里（数据目录），不会上传到任何第三方

### 配置说明

> 🎮 **零配置开箱即用，下面这些全部留空也能跑。** 平台账号 / 画质等内容配置建议直接在管理后台「🌐 源管理」的模块卡片里设置；其余进阶项也可在后台「设置」页改，不必记环境变量。
...
> 💡 平台账号 / 画质 / H.265 等在 **管理后台 →「🌐 源管理」** 对应模块卡片里改，保存即时生效；管理页路径、更新间隔等在 **「设置」** 页；完整环境变量见下方折叠表。
...
| menableClientDispatch | false | boolean | 客户端就近取流：**服务器与观看设备不同运营商**（如服务器移动宽带、电视联通网络）导致抓取频道跨网卡顿 / 播不了时设 `true`<br>开启后播放地址由播放器所在网络就近选择 CDN 节点；同网部署无需开启 |
| mupdateInterval | 8 | number | 节目单 / 源更新间隔（小时），不建议过短 |
| mrefreshToken | true | boolean | 是否每月自动续期平台 token<br>**可能导致封号**，可设 `false` 关闭 |
| mblank | false | boolean | 空白模式总开关，设 `true` 后下面三项内容开关**默认翻转为关**（一行得到空白部署）<br>细粒度开关显式设值时优先于本项，如 `mblank=true` + `menableMigu=true` 可单独保留抓取核心 |
| menableMigu | true | boolean | 是否启用咪咕抓取模块（CCTV/卫视 + 体育赛事 + EPG + token 续期）<br>设 `false` 后仅分发其它源；**体育赛事、回看、对应 EPG 随之不可用** |
...
> 说明：除 `mdataDir`/`mport` 外，以上多数项也可在管理后台修改（模块相关项在「🌐 源管理」模块卡片，平台级项在「设置」页）；后台保存即时生效（端口与更新间隔需重启）。**升级用户注意**：环境变量与旧配置继续有效，首次启动会自动迁移到模块配置，无需手动改动。
...
默认会自带平台抓取模块、内置单频道源（纬来体育 / Red Bull / 4K 卫视等）和内置订阅源（精选频道）。如果你只想用自己的源、要一个干净的环境（也避免自建源与自带源**同名分组撞车**），可以用内容开关把它们关掉。
...
也可在 **管理后台 →「🌐 源管理」** 直接开关各模块 / 内置源卡片，保存后自动重新生成播放列表、即时生效（无需重启）。

> ⚠️ 关闭抓取核心（`menableMigu=false`）后，**体育赛事、回放、对应 EPG（节目单）将不可用**，历史播放列表里的对应频道直链也会失效——这是纯频道管理模式的预期表现。
>
...
| **极空间极影视、部分老电视盒**（列表正常但播放一直转圈） | **兼容版订阅**（`/interface.m3u?relay=1`，后台订阅栏可一键复制） |

> 兼容版说明：少数播放器不跟随 302 跳转，兼容版改由服务器直出播放清单，专治「列表正常、播放转圈失败」；其他播放器用标准订阅即可。遇到新的不兼容播放器欢迎提 issue 补充此表。
...
本项目提供可视化的 Web 管理后台，四个页签管完全部：

- **📺 我的频道** — 频道编排（分组 / 隐藏 / 排序 / 重命名）、多配置档、订阅地址复制
- **🌐 源管理** — 全部内容来源一页聚合：抓取模块、内置源、精选频道订阅、自定义源
- **⚙️ 设置** — 系统配置 / 频道别名 / 自动分组 / EPG 聚合 / 用户管理 五个二级标签
- **ℹ️ 关于我们** — 版本信息与检查更新
...
所有内容来源以卡片网格聚合在一页，每张卡片一个来源：

- **🧩 抓取模块**：每个内容平台一张卡片，卡片上有**独立开关**（关掉即从播放列表移除，互不牵连）、**「配置」按钮**（账号 / 画质 / 频道来源等，支持扫码登录）与**健康状态**（上次抓取时间 / 频道数 / 报错原因）
- **📦 内置源 / 精选频道**：随项目分发的单频道源与订阅源，各自独立开关
- **👤 自定义源**：你自己添加的直连 / 抓取 / 订阅源

#### 添加自定义源的三种方式

**方式一：自动抓取** — 填写包含播放器的网页地址，系统自动访问网页、提取 m3u8 播放地址，支持独立自动刷新保持链接有效。适合网页里有在线播放器、播放地址会过期的源。

**方式二：手动输入** — 直接填写已知的 m3u8 地址，立即生效。适合播放地址长期稳定的源。

**方式三：订阅导入（批量）** — 填写 m3u/m3u8/txt 播放列表的 URL（或本地文件 / 粘贴内容），一次导入全部频道，自动解析名称 / 分组 / 台标，支持定时自动刷新与「默认分组」。适合第三方维护的频道列表。

#### 管理现有源

- **修改 / 删除 / 启用禁用**：卡片上直接操作，保存后 1-2 秒内自动重新生成播放列表（快速模式，无需重新抓取平台数据）
- **🩺 检测失效**：订阅 / 直连源可一键并发探测全部频道连通性，失效频道可一键隐藏
- **排序**：▲/▼ 调整自定义源顺序，决定播放器里同名频道「源1/源2」的先后

**注意事项：**
- ⚠️ 自动抓取需要服务器能访问目标网页
- ⚠️ 部分网页可能有反爬虫机制，导致抓取失败
- ✅ 外部源配置保存在数据目录 `external-sources.json` 文件中
